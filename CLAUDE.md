...
- `npm run nse:deals:guarded` — Same writer, but runs `scripts/nse-deals-guard.mjs` first: skips weekends and NSE trading holidays (listed in `scripts/nse-holidays.txt`, evaluated in IST) so a scheduler doesn't fetch on closed days. Pass `--force`/`NSE_FORCE=1` to the guard to bypass. Prefer this for any scheduled run; keep the holiday list current against the official NSE calendar.
