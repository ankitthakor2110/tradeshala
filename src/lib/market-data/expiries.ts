...
// the same nearest expiry. Returns the live list when a provider responds.

export type ExpirySource = "dhan" | "upstox" | "mock";
...
function generateExpiries(symbol: string): string[] {
  ...
  return [...new Set([...weekly, ...monthly])].sort();
}
...
/** Live expiries (Dhan → Upstox), else the generated fallback. */
export async function getExpiries(
  symbol: string
): Promise<{ expiries: string[]; source: ExpirySource }> {
  const sym = symbol.toUpperCase();

  const dhan = await fetchDhanExpiries(sym);
  if (dhan) return { expiries: dhan, source: "dhan" };

  const upstox = await fetchUpstoxExpiries(sym);
  if (upstox) return { expiries: upstox, source: "upstox" };

  return { expiries: generateExpiries(sym), source: "mock" };
}
